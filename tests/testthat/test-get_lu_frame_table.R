...
test_that("`get_lu_frame_table()` works when the top_n argument is set to TRUE", {
  expect_output(str(get_lu_frame_table(df = phd_data_metaphor, top_n = TRUE, metaphor = "liquid", limit = 5)), "(tbl(_df)?|data\\.frame|tibble)")
})
